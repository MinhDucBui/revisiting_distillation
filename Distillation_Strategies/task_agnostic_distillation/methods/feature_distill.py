...
def embedding_mse(hidden_student, hidden_teacher):
    new_teacher_hidden = [hidden_teacher[0]]
    new_student_hidden =  [hidden_student[0]]
    for student_hidd, teacher_hidd in zip(new_student_hidden, new_teacher_hidden):
        loss_tmp = F.mse_loss(student_hidd, teacher_hidd)
        loss_hidden = loss_tmp
    return loss_hidden
...
def predictions_ce(prediction_score_st, prediction_score_teacher, temperature=1):
    
    ce_loss = soft_cross_entropy(prediction_score_st / temperature, 
                                 prediction_score_teacher / temperature)
    return ce_loss
...
def att_val_frame(teacher, student, args, batch, global_step, wandb, projector=None, eval=False):
    ...
    mlm_loss, attentions_st, qkv_st, hidden_student, prediction_score_st = \
            student.forward(batch, output_attentions=True, output_qkv=True, output_loss=True, output_hidden_states=True)

    if args.method == 'tinybert':
        loss_emb = embedding_mse(attentions_st, attentions_teacher)
        loss_att = att_mse(attentions_st, attentions_teacher, args.layer_selection)
        loss_hidden = hidden_mse(hidden_student, hidden_teacher, args.layer_selection)
        total_loss = loss_emb + loss_att + loss_hidden
        if master_process(args):
            wandb.log({f"{log}/loss": total_loss}, step=global_step)
            wandb.log({f"{log}/loss_emb": loss_emb}, step=global_step)
            wandb.log({f"{log}/loss_att": loss_att}, step=global_step)
            wandb.log({f"{log}/loss_hidden": loss_hidden}, step=global_step)
            wandb.log({f"{log}/loss_mlm": mlm_loss}, step=global_step)
    if args.method == 'distillbert':
        loss_preds = predictions_ce(prediction_score_st, prediction_score_teacher)
        total_loss = loss_preds + mlm_loss
        if master_process(args):
            wandb.log({f"{log}/loss": total_loss}, step=global_step)
            wandb.log({f"{log}/loss_preds": loss_preds}, step=global_step)
            wandb.log({f"{log}/loss_mlm": mlm_loss}, step=global_step)

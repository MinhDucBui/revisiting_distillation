...
def get_layer_norm_type(config):
    if hasattr(config, "layernorm_embedding"):
        ...
        return LAYER_NORM_TYPES[layer_norm_type]
...
class BertIntermediate(nn.Module):
    def __init__(self, config):
        ...
        if hasattr(config, "fused_linear_layer"):
...
class BertLMPredictionHead(nn.Module):
    def __init__(self, config, bert_model_embedding_weights):
        ...
        if hasattr(config, "sparse_mask_prediction"):
...
class BertPreTrainedModel(PreTrainedModel):
    ...
    def from_pretrained_customized(cls, pretrained_model_name_or_path, *inputs, **kwargs):
        ...
        if hasattr(config, "hugging_face") and config.hugging_face:
